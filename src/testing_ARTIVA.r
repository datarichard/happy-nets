...
##### Get a list of complete variable differences ####
get.diffs <- function(df.list, varnames) {
  ...
  for (variable in varnames) {
    ...
    # Extract
    tb <- joinframes(df.list, variable)
    
    # Clean
    tb[which(tb <= 0, arr.ind = TRUE)] <- NA
    tb <- tb[complete.cases(tb),]
    # df[,2:length(df)] <- scale(df[,-1])
    # colnames(df) <- c('xwaveid',letters[1:length(df)-1])
    mat <- diff(t(as.matrix(tb[,2:15])))
    mat <- rbind(rep(0, ncol(mat)), mat)
    mat <- scale(mat)
    #mat <- t(mat)
    tb[,2:length(tb)] <- t(mat)
    colnames(tb) <- c('xwaveid',letters[1:length(tb)-1])
    #colnames(mat) <- letters[1:14]
    #rownames(ans) <- df[['xwaveid']]
    
    # Load
    var.list <- append(var.list, list(tb))
  }
...
diff.list <- get.diffs(hildadata, varname)
complete.subj <- diff.list[[1]]['xwaveid'] # table of xwaveids with complete records


#### Test each subject in ARTIVA ####
targets <- c("ghmh")
parents <- names(diff.list)
results <- list()
for (s in seq(3798)) {
  print(s)
  #### recast the variable list into a subject.df ####
  subj.mat <- matrix(nrow = 6, ncol = 14)
  for (i in seq(length(diff.list))) {
    subj.mat[i,] <- as.matrix(diff.list[[i]][s,2:15])
  }
  row.names(subj.mat) <- names(diff.list)
...
ans <- collated.df[cbind(cp.subj$xwaveid, cp.subj$waveid)] # generate answers
hist(ans)
